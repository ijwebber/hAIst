...
public class CameraControlPlayer : MonoBehaviourPunCallbacks
{
    ...
    void Update()
    {
        ...
        if (isFollowing)
        {
            ...
            if (Physics.Linecast(player.transform.position, cameraTransform.position, out hit, (1 << 8), QueryTriggerInteraction.UseGlobal))
            {
                ...
                if (obstruction.tag == "hideObject" && currentObject[0] == name)
                {   // case where we are hidden behind a wall but we don't move to another wall
                    //obstruction.GetComponent<Renderer>().enabled = false;  
                    // SetAlpha(0.5F);     
                }
                else if (obstruction.tag == "hideObject" && currentObject[0] == "")
                { // case where we are hidden behind a wall but the array is "empty"
                    //obstruction.GetComponent<Renderer>().enabled = false;
                    // SetAlpha(0.5F);  
                    currentObject[0] = name;
                }
                else if (obstruction.tag == "hideObject" && currentObject[0] != name)
                { // case where we are hidden behind a new wall to the one in the array
    ...
    {            // need to merge these together by creating maybe just one object material class?
